...
Figure 6 compares the forecasted values for Alaska with the actual values from the test period. The models are unable to reproduce not just the higher peaks, but also miss the troughs. This shows that the wave power is highly unpredictable, at least at the site of Cook Inlet. Thus, this would require further investigation into other possible sites if we were to consider Alaska as a potential source for tidal energy development.

```{r, fig.cap='Figure 6. Comparing Forecasted Values with Actuals - Alaska'}
#actual test data
