---
title: "Forecasting Tidal Power in Alaska, Maine and Florida"
author: "Ananya Aggarwal, Rosie Wu, Sameer Swarup (https://github.com/Ananya-2809/Swarup_Wu_Aggarwal_ENV797_TSA_FinalProject.git)"
date: "2025-04-25"
output:
  html_document:
    df_print: paged
...
Here is the head of our Maine dataset:
...
From figure 6, we see a significant spike at lag 1 in both ACF and PACF plots - this is similar to our ACF and PACF plots for Maine where tidal power does not show strong correlation with recent months. Also, the significant spikes at lag 12 in both ACF and PACF indicate a strong seasonal component (supported by Figure 8), suggesting that our 3 chosen models should fit well as they model complex seasonality well. Figure 7 shows that much like Maine, there is random noise that our decomposition did not account for, and that future analyses should include exogenous variables to better forecast tidal power.
...
```{r, fig.cap='Figure 5. Time Series plot of monthly mean wave power in Alaska'}
...
```{r,fig.cap='Figure 6. ACF and PACF plots of monthly mean wave power in Alaska'}
...
```{r, fig.cap='Figure 7. Time series plot of decomposed monthly wave power series (Alaska)'}
#decomposing the time series
...
From Table 2, we see significantly lower RMSE and MAPE values, suggesting that perhaps our models are better able to capture monthly mean tidal power trends in Cook Inlet, Alaska. However, Figure 8 paints a different picture.
...
Figure 8 compares the forecasted values for Alaska with the actual values from the test period. The models are unable to reproduce not just the higher peaks, but also miss the troughs. It is likely that our models capture the seasonality of tidal power in Cook Inlet well but not its fluctuations.
...
```{r, fig.cap='Figure 8. Comparing Forecasted Values with Actuals - Alaska'}
#actual test data
...
```{r, fig.cap='Figure 9. Time Series plot of monthly mean wave power in Florida'}
...
```{r, fig.cap='Figure 10. ACF and PACF plots of monthly mean wave power in Florida'}
#plotting acf and pacf
...
```{r, fig.cap='Figure 11. Time series plot of decomposed monthly wave power series (Florida)'}
#decomposing the time series
